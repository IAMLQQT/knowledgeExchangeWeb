...
const { likes, user } = require('../models/models');
...
exports.addForum = catchAsync(async (req, res, next) => {
  ...
  const { forum_name, created_at, forum_description } = req.body;
...
exports.getForumById = catchAsync(async (req, res, next) => {
...
exports.getAllForums = catchAsync(async (req, res, next) => {
  ...
  const forum_status = parseInt(req.query.forum_status) || "0";
  const whereCondition = {};
  if (forum_status !== undefined) {
    whereCondition.forum_status = forum_status;
  }
  ...
  res.status(200).json({
    status: "success",
    data: forums, // Trả về mảng các forums trực tiếp
  });
});
...
exports.getPostForum = catchAsync(async (req, res, next) => {
  ...
  if (postsInForum.length === 0) {
    return res.status(200).json({ status: 'success', message: "The forum has no posts yet." });
  }
...
exports.createPostToForums = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    for (const tagName of tagList) {
      ...
      await tags_posts.create(
        { post_id: post.post_id, tag_id: tag.tag_id },
        { transaction }
      );
    }

    // Cập nhật post_count của forum
    const forumToUpdate = await forum.findByPk(forum_id, { transaction });

    if (!forumToUpdate) {
      throw new AppError("Forum not found!", 404);
    }

    // Tăng giá trị post_count
    forumToUpdate.post_count += 1;
    await forumToUpdate.save({ transaction });
