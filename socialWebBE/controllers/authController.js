...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (accountStatusMessages[userInfo.account_status]) {
    return next(new AppError(accountStatusMessages[userInfo.account_status], 401, userInfo.account_status));
  }
  //if everything ok, send token to client
  const token = signToken(userInfo.accountID, userInfo.passwordVersion, userInfo.RoleID, userInfo.account_status);
  //Seend cookie
  const cookieOptions = {
    expires: new Date(
